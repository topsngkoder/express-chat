...
- [x] **G1**: Hover‑иконки (desktop) для outgoing: карандаш + крестик, 16×16, hit‑area 32×32, видимость на hover/focus.
- [x] **G2**: Long‑press (touch) 450ms: контекстное меню “Редактировать/Удалить”, закрытие по клику вне.
- [x] **G3**: Confirm‑modal удаления (текст/кнопки/деструктивность) + обработка ошибок.
- [x] **G4**: Интеграция с Composer: вход в editMode, подстановка текста, выход, Esc=cancel.
