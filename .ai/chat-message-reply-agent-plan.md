...
- [x] **E2**: Добавить `Ответить` в touch context menu.
...
- [x] **G1**: Рендерить цитату внутри reply-сообщения.
- [x] **G2**: Соблюсти порядок: quote -> image -> text -> time/status.
- [x] **G3**: Соблюсти правила preview и цвета автора.
- [x] **G4**: Сделать block кликабельным только когда `isNavigable = true`.
